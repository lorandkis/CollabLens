...
echo "â³ Waiting for MySQL at ${DB_HOST}:${DB_PORT} (timeout: ${DB_WAIT_TIMEOUT}s)..."
deadline=$((SECONDS + DB_WAIT_TIMEOUT))
while ! mysqladmin ping -h "${DB_HOST}" -P "${DB_PORT}" -u"${DB_USER}" -p"${DB_PASS}" --silent; do
  if (( SECONDS >= deadline )); then
    echo "âš ï¸  MySQL not ready after ${DB_WAIT_TIMEOUT}s. Continuing startup anyway."
    break
  fi
  sleep 2
done
...
if [ -f /var/www/html/createTables.php ]; then
  echo "âœ… Attempting to run createTables.php..."
  if ! php /var/www/html/createTables.php; then
    echo "âš ï¸  createTables.php failed (will not block Apache). Check logs."
  fi
else
  echo "â„¹ï¸  /var/www/html/createTables.php not found. Skipping."
fi
